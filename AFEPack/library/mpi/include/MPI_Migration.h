...
namespace Migration {
  namespace details {
    ...
    template <int DOW> void
      clear_data_buffer(HGeometry<0,DOW>& geo) {
      geo.buffer.clear();
    }
    template <int DOW> void
      clear_data_buffer(HGeometry<1,DOW>& geo) {
      geo.buffer.clear();
      for (u_int i = 0;i < geo.n_vertex;++ i) {
        clear_data_buffer(*geo.vertex[i]);
      }
      if (geo.isRefined()) {
        for (u_int i = 0;i < geo.n_child;++ i) {
          clear_data_buffer(*geo.child[i]);
        }
      }
    }
    template <class GEO> void
      clear_data_buffer(GEO& geo) {
      geo.buffer.clear();
      for (u_int i = 0;i < geo.n_boundary;++ i) {
        clear_data_buffer(*geo.boundary[i]);
      }
      if (geo.isRefined()) {
        for (u_int i = 0;i < geo.n_child;++ i) {
          clear_data_buffer(*geo.child[i]);
    ...
    struct buffer_trait {
      ...
      friend AFEPack::ostream<>&
      operator<<(AFEPack::ostream<>& os,
      ...
      friend AFEPack::istream<>&
      operator>>(AFEPack::istream<>& is,
                 buffer_trait& bt) {
    ...
    template <class HGEO, class T>
      ...
      for (u_int i = 0;i < geo.n_boundary;++ i) {
        export_property(*geo.bound[i], data_id, pid, flag);
      }
    ...
    template <class HGEO, class T>
      ...
      for (u_int i = 0;i < geo.n_boundary;++ i) {
        import_property(*geo.bound[i], data_id, pid, flag);
      }
  ...
  template <class HTREE> void
    ...
    for (;the_ele != end_ele;++ the_ele) {
      ...
      details::clear_data_buffer(*p_geo);
    }
  }
