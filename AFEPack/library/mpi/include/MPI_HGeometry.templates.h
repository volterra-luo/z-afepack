...
#undef GDIM
  ...
  std::vector<AFEPack::ostream<> > proc_os(n_rank);
  ...
  for (int i = 0;i < n_rank;++ i) { /// 对进程进行循环
    if (proc_flag[i] == false) continue;
    AFEPack::istream<> proc_is(proc_buf_in[i]);
...
TEMPLATE
void THIS::renumerateRootElement(void (*f)(double, double, double,
                                           double&,double&,double&))
{
  ...
  for (int i = 0;i < n_ele;++ i, ++ the_ele) {
    ...
    for (int j = 0;j < n_vtx;++ j) {
      Point<this_t::dow>& pnt = *ele.vertex[j];
      x[i] += pnt[0]; y[i] += pnt[1];
      if (this_t::dow == 3) z[i] += pnt[2];
    }
    x[i] /= n_vtx; y[i] /= n_vtx;
    if (this_t::dow == 3) z[i] /= n_vtx;
  }
