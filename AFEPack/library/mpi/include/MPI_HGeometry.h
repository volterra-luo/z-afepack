...
#undef GDIM
  ...
  void renumerateRootElement(void (*f)(double, double, double,
...
#undef SYNC_DATA
   ...
   */
  template <class FOREST>
    class BirdViewSet : public std::set<BirdView<FOREST>*> {
  private:
    typedef BirdView<FOREST> birdview_t;
    typedef std::set<birdview_t*> base_t;
  public:
  ...
  public:
    void add(birdview_t& bv) { base_t::insert(&bv); }
    void erase(birdview_t& bv) { base_t::erase(&bv); }
