...
#undef GDIM
  ...
  void config(birdview_t& ir_mesh,
              LOADER& loader,
              u_int (LOADER::*value)(const GeometryBM&)) {
  ...
  void config(birdview_t& ir_mesh,
              u_int (*value)(const GeometryBM&) = &_default_element_loading) {
    if (&(ir_mesh.getForest()) != _forest) {
      std::cerr << "The mesh should be on the same forest of the HLoadBalance." 
                << std::endl;
      abort();
  ...
  static u_int _default_element_loading(const GeometryBM&) { return 1; }
  //@}
...
#undef SYNC_DATA
  ...
  template <int D> void 
    save_dim_orphans(boost::archive::HGeometry_oarchive<this_t>& oa) {
