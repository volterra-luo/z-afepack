  ...
  SimplestMesh<DIM,DOW>::generateMesh(Mesh<DIM,DOW>& m)
{
  ...
  for (i = 0, p = -1;i < n;i ++) {
	...
	for (l = m.n_geometry(j);l > 0;l --) {
	  if (isSame(g, m.geometry(j, l - 1))) {
	    geo_img[j][k] = l - 1;
	    break;
	  }
	}
...
template <int DIM, int DOW>
  void Mesh<DIM,DOW>::renumerateElementHSFC(void (*f)(double,double,double,double&,double&,double&))
{
  std::cerr << "Renumerating element of the mesh using Hibert space curve filling ..." 
            << std::flush;
  int n_ele = n_geometry(DIM);
  std::vector<double> x(n_ele, 0.0), y(n_ele, 0.0), z(n_ele, 0.0);
  for (int i = 0;i < n_ele;++ i) {
    GeometryBM& ele = geometry(DIM, i);
    int n_vtx = ele.n_vertex();
    for (int j = 0;j < n_vtx;++ j) {
      int vtx_idx = geometry(0, ele.vertex(j)).vertex(0);
      Point<DOW>& pnt = point(vtx_idx);
      x[i] += pnt[0]; y[i] += pnt[1];
      if (DOW == 3) z[i] += pnt[2];
    }
    x[i] /= n_vtx; y[i] /= n_vtx;
  ...
  if (f == NULL) {
    hsfc_renumerate(n_ele, &x[0], &y[0], &z[0], &new_index[0]);
  } else {
    hsfc_renumerate(n_ele, &x[0], &y[0], &z[0], &new_index[0], f);
  }
