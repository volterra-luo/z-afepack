...
#include "Migration.h"
struct HBuffer : public PropertyTable {
...
struct HTools {
  ...
  isRefined(const GEO& geo) const {
    if (geo.isRefined()) {
      return this->isGeometryUsed(*geo.child[0]);
    } else {
      return false;
    }
  }
...
template <int DIM, int DOW=DIM>
  class HGeometry : public HGeometryInfo<DIM>, public HBuffer
  {
  public:
 ...
 */
template <int DOW>
class HGeometry<0,DOW> : public Point<DOW>, public HGeometryInfo<0>, public HBuffer
{
 public:
  enum { dim = 0, dow = DOW };
  typedef HGeometry<dim,dow> this_t;
...
template <int DOW=1>
  class HGeometry<1,DOW> : public HGeometryInfo<1>, public HBuffer
  {
  public:
  enum { dim = 1, dow = DOW };
  typedef HGeometry<0,dow> vertex_t;
...
template <int DOW=2>
  class HGeometry<2,DOW> : public HGeometryInfo<2>, public HBuffer
  {
  public:
  enum { dim = 2, dow = DOW };
  typedef HGeometry<0,dow> vertex_t;
...
template <int DOW=3>
  class HGeometry<3,DOW> : public HGeometryInfo<3>, public HBuffer
  {
  public:
  enum { dim = 3, dow = DOW };
  typedef HGeometry<0,dow> vertex_t;
 ...
 */
template <int DIM, int DOW=DIM>
  class HElement : public HGeometryInfo<DIM>, public HBuffer {
 public:
 enum { dim = DIM, dow = DOW };
...
#ifdef __SERIALIZATION__
  std::vector<std::vector<HBuffer*> > h_geometry_ptr; /// 几何体对HGeometry的反向索引表
#endif
...
#ifdef __SERIALIZATION__
  std::vector<std::vector<HBuffer*> >& h_geometry() { return h_geometry_ptr;}
  const std::vector<std::vector<HBuffer*> >& h_geometry() const { return h_geometry_ptr;}
  HBuffer * h_geometry(int dim, u_int idx) const { return h_geometry_ptr[dim][idx]; }
