                                ...
                                int& n_element_refined)
{
  if (element.value == 0) { // if this element is a leaf element
    ...
    Tools().semiregularizeBoundary(h_element);
...
void THIS::renumerateElement()
{
  ...
  std::vector<HBuffer*> hgp(regular_mesh->h_geometry_ptr[DIM]);
#endif
  for (i = 0;i < n_ele;i ++) {
    GeometryBM& ele = regular_mesh->geometry(DIM,i);
...
TEMPLATE
  void THIS::renumerateElementHSFC(void (*f)(double,double,double,double&,double&,double&))
{
  ...
  for (int i = 0;i < n_ele;++ i) {
    ...
    for (int j = 0;j < n_vtx;++ j) {
      ...
      Point<DOW>& pnt = this->point(vtx_idx);
      x[i] += pnt[0]; y[i] += pnt[1];
      if (DOW == 3) z[i] += pnt[2];
    }
    x[i] /= n_vtx; y[i] /= n_vtx;
  ...
  std::vector<int> old_index(n_ele);
#ifdef __SERIALIZATION__
  std::vector<HBuffer*> hgp(h_geometry_ptr[DIM]);
#endif
  for (int i = 0;i < n_ele;i ++) {
    GeometryBM& ele = this->geometry(DIM,i);
    ele = tmp_ele[new_index[i]];
