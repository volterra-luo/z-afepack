...
void SCL::load_balance()
{
  ...
  std::string save_path("/tmp/.rli.hlb.save");
  std::string load_path("/tmp/.rli.hlb.load");

  system((std::string("rm -rf ") + save_path).c_str());
  system((std::string("rm -rf ") + load_path).c_str());
  ...
  hlb.save_data(save_path, bvs); /// 存储数据
  ...
  MPI::lb_sync_local_data_dir(htree.communicator(), 
        		      save_path, load_path);
  //hlb.write_config_file(load_path); /// 已经可以不要这一行了
  system("sync");

  /// 最后将数据重新载入
  MPI_Barrier(htree.communicator()); /// 同步以后
  hlb.load_data(load_path, bvs); /// 载入数据
...
void SCL::initialize()
{
  ...
  std::string template_path =
    std::string("$AFEPACK_PATH/template/interval:") +
    std::string("$AFEPACK_PATH/template/triangle:") +
    std::string("$AFEPACK_PATH/template/twin_triangle:");
  setenv("AFEPACK_TEMPLATE_PATH", template_path.c_str(), 1);
...
void SCL::adapt_mesh() {
  ...
  /// 如果需要加密的网格数太少，则不做自适应
  int is_rank_adapt = (n_ele_to_refine < ADAPT_RATIO*n_ele)?0:1;
