...
void SCL::adapt_mesh() {
  ...
  /**
   * 下面这一段是本例的重点，在自适应以后对内存进行回收。请注意此处的
   * 代码目的是为了对代码进行测试，在时间使用的时候，在内存够用的情形
   * 下，尽量不要过于频繁地进行内存回收操作。
   */
  std::cout << "Reclaiming memory ... " << std::flush;
  MPI::MemoryReclaimer<SCL::forest_t> mr(htree);
  mr.addIrregularMesh(*ir_mesh);
  mr.reclaim();
